...
\begin{figure}%
    ...
    \caption{\textbf{Graph A}: TPO levels in patients with Aplastic anaemia (AA) versus ITP. There is a clear difference between the two patient groups, apart from 3 outliers within the AA group which have lower TPO and platelet counts. These patients had recieved tacrolimus therapy and subsequently recovered their platelet counts. \textbf{Graph B}: log(TPO) levels versus platelet counts in ITP (red) and AA (blue). The graph shows the \textit{\textbf{MAP}} estimates \emph{(black lines)} and \textit{\textbf{90\% BIC interval}} (dark grey) as well as \textit{\textbf{total sampling area}} (light grey) for the two bayesian models generated from the data. \textbf{Graph C}: Shows the same graph as in B, except the a linear regression model is fitted. This gives a visual demonstration of the difference between these two regression methods}%
    \label{fig:example}%
\end{figure}
...
\paragraph{} The bayesian model used in this study demonstrated a clear non-linear relationship between platelet counts and TPO levels in both ITP and aplastic anaemia (AA) patients (\textbf{\emph{FIGURE 1, Graph B}}). Patients with ITP had consistently lower TPO at all predicted platelet counts when compared to the AA patients. The DIC score for the model was significantly lower than for either linear or polynomial models.
...
\paragraph{} This study demonstrates a non-linear association between platelet counts and TPO. It shows that in ITP, TPO levels are consistently lower than in aplastic anaemia. This suggests that irrespective of platelet count, megakarocyte mass is a significant influence on TPO levels. However, as the platelet counts fall below 50$\times10^9/L$, TPO levels start to rise significantly much close association with the platelet count. One explanation for this is that at the \emph{lower platelet counts} the sponging effect of the platelets has a greater influence on TPO levels than at higher platelet counts. In contrast higher platelet counts it is the megakarocyte mass which is the primary determinant of TPO levels. The observation that TPO agonists appear to cause TPO levels to drop at higher platelet counts might be explained by an expansion of the megakaryocyte pool. Why TPO agonists might increase TPO levels at lower platelet counts is not clear and maybe due to 
