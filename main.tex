...
\paragraph{} This study demonstrates that TPO levels in ITP are consistently lower than aplastic anaemia, irrespective of platelet count but the relationship between platelet counts and TPO is non-linear. These findings suggest that megakaryocyte mass does have a dominant influence on TPO levels although platelets do appear to exert an influence but only once platelet counts fall below 40-50$\times10^9/L$. We also observed that TPO agonists are associated with \emph{higher} TPO levels at \emph{lower} platelet counts (relative to untreated patients) but are then have \emph{lower} TPO levels when platelet counts are high. One explanation for this might be that TPO agonists both \emph{out-compete} natural TPO for MPL-receptors but then also expand the megakaryocyte pool causing greater TPO \emph{sponging}. The relative influence of these two processes on TPO levels may change as the platelet count varies.  
